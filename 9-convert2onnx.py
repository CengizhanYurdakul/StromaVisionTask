...
def main(args):
    ...
    for weightName in tqdm(dirs):
    
        b = args.input_shape[0]
        YOLOv8 = YOLO(os.path.join(path, weightName, "weights/best.pt"))
        ...
        for _ in range(2):
            model(fake_input)
        save_path = os.path.join(path, weightName, "weights/best.pt").replace('.pt', '.onnx')
        with BytesIO() as f:
            torch.onnx.export(
                model,
                fake_input,
